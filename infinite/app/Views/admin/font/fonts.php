    ...
    <div class="col-lg-5 col-md-12">
                    ...
                    <div class="form-group">
                        ...
                        <select name="lang_id" class="form-control" onchange="window.location.href = '<?= adminUrl(); ?>' + '/font-settings?lang='+this.value;">
                            <?php foreach ($languages as $language): ?>
                                <option value="<?= $language->id; ?>" <?= $fontLangId == $language->id ? 'selected' : ''; ?>><?= $language->name; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                        ...
                        <select name="primary_font" class="form-control custom-select">
                            <?php if (!empty($fonts)):
                                foreach ($fonts as $font): ?>
                                    <option value="<?= $font->id; ?>" <?= $settings->primary_font == $font->id ? 'selected' : ''; ?>><?= $font->font_name; ?></option>
                                <?php endforeach;
                            endif; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="secondary_font" class="form-control custom-select" style="width: 100%">
                            <?php foreach ($fonts as $font): ?>
                                <option value="<?= $font->id; ?>" <?= $settings->secondary_font == $font->id ? 'selected' : ''; ?>><?= $font->font_name; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <a href="https://fonts.google.com/" target="_blank" style="float: right;font-size: 16px;"><strong><?= trans("google_fonts"); ?>&nbsp;<i class="icon-arrow-right"></i></strong></a>
            </div>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu options-dropdown">
                                                        ...
                                                        <li>
                                                            <a href="javascript:void(0)" onclick="deleteItem('Admin/deleteFontPost','<?= $font->id; ?>','<?= trans("confirm_item"); ?>');"><i class="fa fa-trash option-icon"></i><?= trans('delete'); ?></a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </td>
